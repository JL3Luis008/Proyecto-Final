...
router.get('/ShippingAddress', authMiddleware, getUserAddresses);

// Obtener la dirección por defecto
router.get('/ShippingAddress/default', authMiddleware, getDefaultAddress);

// Obtener una dirección específica
router.get('/ShippingAddress/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
], validate, authMiddleware, getAddressById);

// Crear una nueva dirección
router.post('/ShippingAddress', addressValidations, validate, authMiddleware, createShippingAddress);

// Actualizar una dirección
router.put('/ShippingAddress/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId'),
  ...addressValidations
...
router.patch('/ShippingAddress/default/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
], validate, authMiddleware, setDefaultAddress);

// Eliminar una dirección
router.delete('/ShippingAddress/:addressId', [
  param('addressId')
    .isMongoId().withMessage('Address ID must be a valid MongoDB ObjectId')
