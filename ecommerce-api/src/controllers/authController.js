...
async function register(req, res) {
  try {
    ...
    await newUser.save();
    res.status(201).json({ displayName, email, phone, role });
  } catch (error) {
    next(error);
  }
}*/
...
async function register(req, res, next) {
  try {
    const { displayName, email, password, phone, role = 'guest' } = req.body;

    // Validar que el rol esté en la lista permitida
    const allowedRoles = ["Administrator", "Customer", "Guest"];
