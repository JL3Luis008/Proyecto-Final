...
import { checkEmail, login, register, refreshToken } from "../authController.js";
...
describe("AuthController", () => {
  ...
  describe("login", () => {
    it("should login user successfully and return token", async () => {
      ...
      expect(res.json).toHaveBeenCalledWith({ token: "mockToken123", refreshToken: "mockToken123" });
      expect(next).not.toHaveBeenCalled();
    });
  ...
  describe("refreshToken", () => {
    // AU-16
    it("AU-16 Token válido → responde con nuevo access + refresh token", async () => {
      req.body = { refreshToken: "validRefreshToken" };

      const decoded = { userId: "user123", displayName: "Test User", role: "customer" };
      // jwt.verify llama al callback con (null, decoded)
      mockJwtSign
        .mockReturnValueOnce("newAccessToken123")
        .mockReturnValueOnce("newRefreshToken123");

      // Necesitamos mockear jwt.verify para que use el callback con decoded
      // El mock está como vi.fn().mockReturnValue para sign, pero verify es callback-based
      // Redefinimos verify en el módulo mockeado usando mockImplementation
      const { default: jwt } = await import("jsonwebtoken");
      jwt.verify = vi.fn().mockImplementation((_token, _secret, cb) =>
        cb(null, decoded)
      );
    ...
    // AU-17
    it("AU-17 Sin refreshToken en body → 401", async () => {
    ...
    // AU-18
    it("AU-18 Token inválido o expirado → 403", async () => {
      req.body = { refreshToken: "expiredOrInvalidToken" };
    ...
    // AU-19 — refreshToken: el try/catch sólo atraparía errores síncronos previos al verify
    // En la implementación actual, si body.refreshToken existe el flujo va a jwt.verify
    // que es callback-based y no lanza excepciones fuera del catch.
    // Documentado: no hay ruta de código que llame next(error) en esta función.
    it("AU-19 [DECISION] refreshToken no tiene ruta next(error) — jwt.verify es callback-based", () => {
      // Este test documenta intencionalmente que AU-19 no aplica al código actual.
      // Si en el futuro se refactoriza refreshToken a async/await con jwt.verify promisificado,
      // este test deberá actualizarse.
      expect(true).toBe(true); // placeholder — decisión de diseño documentada
    });
  });
});
